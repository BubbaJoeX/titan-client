...
function Get-PackageFile {
    ...
    try {
        ...
        if ($curl) {
            # --ssl-no-revoke: schannel cannot always reach the CRL/OCSP endpoint (VPN/proxy/offline
            # revocation server) and fails the handshake with CRYPT_E_NO_REVOCATION_CHECK (0x80092012).
            # Integrity is still enforced below via Assert-MicrosoftPackage (SHA-256 + Authenticode).
            & $curl.Source --fail --location --ssl-no-revoke --retry 3 --retry-delay 2 --output $partial ([string]$Package.url)
