...
$treFiles = @(
    Get-ChildItem -LiteralPath $clientRootPath -Filter "*.tre" -File -ErrorAction SilentlyContinue
    Get-ChildItem -LiteralPath (Join-Path $clientRootPath "tres") -Filter "*.tre" -File -ErrorAction SilentlyContinue
    Get-ChildItem -LiteralPath (Join-Path $clientRootPath "tres") -Filter "*.titanpak" -File -ErrorAction SilentlyContinue
)
if ($treFiles.Count -eq 0) {
    throw "ClientRoot does not contain TRE or titanpak asset files: $clientRootPath"
}
...
$runtimeFiles = @(
    [pscustomobject]@{
        Source = Join-Path $repoRoot "src\build\win32\x64\$Configuration\SwgTitan_$suffix.exe"
        Name   = "SwgTitan_$suffix.exe"
    },
    [pscustomobject]@{
...
foreach ($file in $runtimeFiles) {
    ...
    if ($file.Name -like "SwgTitan_*.exe") {
        $bytes = (Get-Item -LiteralPath $file.Source).Length
        if ($bytes -lt $minClientExeBytes) {
            throw ("Refusing to stage suspiciously small client executable ({0:N0} bytes, expected at least {1:N0}): {2}. Run a full SwgClient rebuild (/t:Rebuild) before staging." -f $bytes, $minClientExeBytes, $file.Source)
        }
    }
}
...
$miscDir = Join-Path $clientRootPath "misc"
$overrideTemplate = @"
# Loaded after client.cfg by ClientMain::installConfigFileOverride.
# Set loginServerAddress0 to the host where LoginServer is actually running.

[ClientGame]
    loginServerAddress0=localhost
    loginServerPort0=44453
    loginClientID=
    loginClientPassword=
    autoConnectToLoginServer=true
"@
if (-not (Test-Path -LiteralPath $miscDir -PathType Container)) {
    New-Item -ItemType Directory -Path $miscDir -Force | Out-Null
}
$overridePath = Join-Path $miscDir "override.cfg"
if (-not (Test-Path -LiteralPath $overridePath -PathType Leaf)) {
    [IO.File]::WriteAllText($overridePath, $overrideTemplate + [Environment]::NewLine, $utf8NoBom)
    Write-Host "Created default misc/override.cfg (edit loginServerAddress0 for your LoginServer host)."
}
