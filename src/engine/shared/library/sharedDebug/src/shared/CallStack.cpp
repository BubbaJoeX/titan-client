...
void CallStack::debugPrint() const
{
	...
	for (size_t i = 2; i < CALLSTACK_DEPTH; ++i)
	{
		if (DebugHelp::lookupAddress(m_callStack[i], libName, fileName, sizeof(fileName), line))
			DEBUG_REPORT_PRINT(true, (" (%p) %s(%d) : caller %d\n", reinterpret_cast<void const *>(m_callStack[i]), fileName, line, i - 1));
		else
			DEBUG_REPORT_PRINT(true, ("  unknown(%p) : caller %d\n", reinterpret_cast<void const *>(m_callStack[i]), i - 1));
	}
}
...
void CallStack::debugLog() const
{
	...
	{
		if (DebugHelp::lookupAddress(m_callStack[i], libName, fileName, sizeof(fileName), line))
			DEBUG_REPORT_LOG(true, ("  (%p) %s(%d) : caller %d\n", reinterpret_cast<void const *>(m_callStack[i]), fileName, line, i - 1));
		else
			DEBUG_REPORT_LOG(true, ("  unknown(%p) : caller %d\n", reinterpret_cast<void const *>(m_callStack[i]), i - 1));
	}
}
