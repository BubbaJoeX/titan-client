...
int StdioFile::write(unsigned int num_bytes, const void* source_buffer)
{
    assert(m_file != NULL);
    m_justWrote = true;
...
std::shared_ptr<AbstractFile> StdioFileFactory::createFile(const char *fileName, const char *openType)
{
    if(!fileName || !openType)
        ...
        return std::make_shared<StdioFile>(fileName, openType);
}
