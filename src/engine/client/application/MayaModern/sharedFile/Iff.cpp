...
bool IffNamespace::isValid(byte const *memory, int length)
{
    ...
    while (length > 0)
    {
        // read the block's tag (FORM or the chunk name)
        Tag blockTag(0);
...
{
    std::shared_ptr<AbstractFile> file = std::make_shared<StdioFile>(fileName, "rb");
    if (!file)
        return false;
...
{
    std::shared_ptr<AbstractFile> file = std::make_shared<StdioFile>(newFileName, "rb");
...
void Iff::open(std::shared_ptr<AbstractFile> file)
{
    open(file, 0);
}
...
void Iff::open(std::shared_ptr<AbstractFile> file, char const * const newFileName)
{
    close();
...
bool Iff::enterForm(Tag name, bool validateName, bool optional)
{
    ...
    if (!optional)
    {
        ...
        char buf[256];
        _snprintf( buf, sizeof(buf), "enter form [%s] failed", tagname );
...
void Iff::exitChunk(Tag name, bool mayNotBeAtEndOfChunk)
{
    ...
    IFF_FATAL(getFirstTag(stackDepth-1) != name, ("exit chunk with incorrect name"));
...
void Iff::exitChunk(bool mayNotBeAtEndOfChunk)
{
    ...
    // advance beyond the chunk we were in
...
void Iff::read_floatVector(int count, Vector *array)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        array[i].x = static_cast<float>(f);

        read_misc(&f, isizeof(f));
        array[i].y = static_cast<float>(f);

        read_misc(&f, isizeof(f));
        array[i].z = static_cast<float>(f);
    }
}
...
void Iff::read_floatTransform(int count, Transform *array)
{
    ...
    for (int i = 0; i < count; ++i)
        for (int y = 0; y < 3; ++y)
            ...
            {
                read_misc(&f, isizeof(f));
                array[i].matrix[y][x] = static_cast<float>(f);
            }
}
...
Transform Iff::read_floatTransform(void)
{
    ...
    for (int y = 0; y < 3; ++y)
        ...
        {
            read_misc(&f, isizeof(f));
            t.matrix[y][x] = static_cast<float>(f);
        }
...
{
    const float w = read_float();
    const float x = read_float();
    const float y = read_float();
    const float z = read_float();
...
VectorArgb Iff::read_floatVectorArgb(void)
{
    ...
    read_misc(&f, isizeof(f));
    a = static_cast<float>(f);

    read_misc(&f, isizeof(f));
    r = static_cast<float>(f);

    read_misc(&f, isizeof(f));
    g = static_cast<float>(f);

    read_misc(&f, isizeof(f));
    b = static_cast<float>(f);
