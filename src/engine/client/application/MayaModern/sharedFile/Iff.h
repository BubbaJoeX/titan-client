...
public:
    ...
    void open(std::shared_ptr<AbstractFile> file);
    void open(std::shared_ptr<AbstractFile> file, char const * fileName);
    void close(void);
    ...
    /** Read int16 in big-endian (network) byte order. Use for IFF chunk data when format is big-endian. */
    int16       read_int16_be(void);
    /** Read uint32 in big-endian byte order. */
    uint32      read_uint32_be(void);
    /** Read float in big-endian byte order. */
    ...
    void	read_wchar_t(int count, wchar_t *array);
...
inline int16 Iff::read_int16_be(void)
{
    uint16 i;
    read_misc(&i, isizeof(i));
    return static_cast<int16>(static_cast<uint16>((i & 0xFFU) << 8 | (i >> 8)));
}

inline uint32 Iff::read_uint32_be(void)
{
    uint32 i;
    read_misc(&i, isizeof(i));
    return (i & 0xFFU) << 24 | (i & 0xFF00U) << 8 | (i & 0xFF0000U) >> 8 | (i >> 24);
}

inline float Iff::read_float_be(void)
{
    uint32 i;
    read_misc(&i, isizeof(i));
    i = (i & 0xFFU) << 24 | (i & 0xFF00U) << 8 | (i & 0xFF0000U) >> 8 | (i >> 24);
    float f;
    memcpy(&f, &i, sizeof(f));
    return f;
}
...
inline float Iff::read_float(void)
{
    float  f;
...
inline void Iff::read_wchar_t(int count, wchar_t *array)
{
    read_misc(array, count * isizeof(*array));
}
