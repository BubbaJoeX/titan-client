...
const float   Vector::NORMALIZE_THRESHOLD(CONST_REAL(0.00001));

const float   Vector::NORMALIZED_EPSILON(CONST_REAL(0.00001));

// If M is between 1-e and 1+e, then M^2 is between 1-2e+e^2 and 1+2e+e^2.

static const float   NORMALIZED_RANGE_SQUARED_MIN = (1.0f - (2.0f * Vector::NORMALIZED_EPSILON)) + (Vector::NORMALIZED_EPSILON * Vector::NORMALIZED_EPSILON);
static const float   NORMALIZED_RANGE_SQUARED_MAX = (1.0f + (2.0f * Vector::NORMALIZED_EPSILON)) + (Vector::NORMALIZED_EPSILON * Vector::NORMALIZED_EPSILON);
...
const Vector Vector::findClosestPointOnLine(const Vector &line0, const Vector &line1, float *t) const
{
    ...
    const float r  = (*this - line0).dot(delta) / delta.magnitudeSquared();
    *t = r;
...
const Vector Vector::findClosestPointOnLineSegment(const Vector & startPoint, const Vector & endPoint) const
{
    ...
    const float deltaMagnitudeSquared = delta.magnitudeSquared();
    if(deltaMagnitudeSquared <NORMALIZE_THRESHOLD)
        return startPoint;

    const float r = clamp(CONST_REAL(0), (*this - startPoint).dot(delta) / deltaMagnitudeSquared, CONST_REAL(1));
...
float Vector::distanceToLine(const Vector &line0, const Vector &line1) const
{
...
float Vector::distanceToLineSegment(const Vector &line0, const Vector &line1) const
{
