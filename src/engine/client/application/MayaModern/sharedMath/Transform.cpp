...
void Transform::yaw_l(
        float radians  // Amount to rotate, in radians
)
{
...
void Transform::rotateTranslate_p2l(const Vector *source, Vector *result, int count) const
{
    ...
    for ( ; count; --count, ++source, ++result)
    {
        const float x = source->x - matrix[0][3];
        const float y = source->y - matrix[1][3];
        const float z = source->z - matrix[2][3];
...
void Transform::ypr_l(float const y, float const p, float const r)
{
    float const cy = cos(y);
    float const sy = sin(y);
    float const cp = cos(p);
    float const sp = sin(p);
    float const cr = cos(r);
    float const sr = sin(r);
