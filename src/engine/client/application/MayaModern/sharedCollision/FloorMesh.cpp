...
bool FloorMesh::dropTest(FloorLocator const & testLoc, float hopHeight, FloorLocator & outLoc) const
{
	// aconite 3/5/22
	//
	// we want this drop test to run with respect to kashyyyk fallthrough because we need it for
	// transitioning from one floor to another where the floors overlap (an example of this is the
	// literally the first bridge at the top of the trail that overlaps with the trees) For Example:
	//
	//	v bridge
	//	______		  v mesh of the tree you should walk onto when going down the stairs
	//		  |_
	//		   |_---------------- <
	//		    |_
	//			  |_		< stairs you end up walking down if this doesn't run (to the abyss)
	//
	// however: i think this may be a problem with some *specific* bridges because of how they are
	// positioned, but the big change with the over dropTest() is what fixed most of the issues
	...
	const float fallHeight = m_objectFloor ? REAL_MAX : 3.0f;
	...
	{
		const float distBelow = std::abs(closestBelow.getOffset());
		const float distAbove = std::abs(closestAbove.getOffset());
...
{
	const Vector point = testLoc.getPosition_l();

	const Line3d line(point,-Vector::unitY);
	...
	else
	{
		// aconite 3/5/22
		//
		// in the context of walking on floors that use regular sweep checks and not a drop test for mesh to mesh,
		// testIntersect() is the block that fails triggering Kashyyyk fallthrough from this block. This has become a
		// slippery slope because we very quickly end up down the block of horrible math in the path of
		// CollisionMesh > Intersect3d > Overlap3d::TestLineTri() ... which just leads us to horrid floating
		// point conversion issues (esp. in vectorLess() method of CollisionUtils). Consequently, we're
		// just going to hack this here for now (and probably forever if we're being realistic) and evaluate
		// whether or not we're in the Rryatt Trail Scene, and if so, we'll just return that it's fine and
		// we don't actually need to fall through.
		//
		// QA Note: I've ran through the entire trail a few times to see if this leads to any weird edge
		// cases of getting caught in the floor from sweep contacts not dropping you when it should or the
		// hop height jump not being computed, and that doesn't seem to be the case, but we'll just have to
		// watch for weird reports about any issues relating to this change.
		if (CollisionWorld::isForceReattachScene())
		{
			return true;
		}
	}
