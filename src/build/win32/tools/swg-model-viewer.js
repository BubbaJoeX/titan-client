...
class SWGModelViewer {
  ...
  async loadTexture(texturePath) {
    ...
    try {
      console.log('[SWGModelViewer] Loading texture:', texturePath);
      const response = await fetch(`/api/models/texture/${encodeURIComponent(texturePath)}`);
      const result = await response.json();

      if (!result.success || !result.data) {
        console.warn('[SWGModelViewer] Failed to load texture:', texturePath);
        return null;
      }
      ...
      console.error('[SWGModelViewer] Error loading texture:', error);
      return null;
    }
  }
  ...
  createTextureFromDDS(base64Data, width, height, format) {
    try {
      ...
      console.log('[SWGModelViewer] Creating texture', {
        width,
        height,
        format,
      ...
      // SWG uses DirectX-style top-left origin, Three.js uses bottom-left
      texture.flipY = true;

      texture.wrapS = THREE.RepeatWrapping;
      texture.wrapT = THREE.RepeatWrapping;
      texture.magFilter = THREE.LinearFilter;
      // Use LinearFilter instead of LinearMipmapLinearFilter since we don't have mipmaps
      ...
      console.log('[SWGModelViewer] Created texture successfully', { width, height, format });
      return texture;
    } catch (error) {
  ...
  createMaterial(primitive, diffuseTexture = null, normalTexture = null, specularTexture = null) {
    // Determine material type based on shader template name
    ...
    // Base material options - match SWG's rendering
    const materialOptions = {
      color: diffuseTexture ? 0xffffff : 0x8899aa,
      side: THREE.DoubleSide, // SWG renders both sides for most objects
      flatShading: false,
      transparent: isAlpha || isTransparent,
      alphaTest: isAlpha ? 0.5 : 0, // SWG uses alpha testing for foliage etc
    };

    let material;

    if (isUnlit || this.options.lightingMode === 'unlit') {
      // Unlit/emissive materials - no lighting calculations
      material = new THREE.MeshBasicMaterial({
        ...materialOptions,
        map: diffuseTexture,
      });
    } else if (hasNormalMap) {
      // Normal mapped material for detailed surfaces
    ...
    // Handle additive blending (glow effects)
    if (isAdditive) {
    ...
    if (diffuseTexture) {
      console.log('[SWGModelViewer] Applying texture to material', {
        textureWidth: diffuseTexture.image?.width,
        textureHeight: diffuseTexture.image?.height,
        shaderTemplate: primitive.shaderTemplate,
        materialType: material.type
      });
    } else {
      console.log('[SWGModelViewer] Creating material without texture', {
        shaderTemplate: primitive.shaderTemplate,
