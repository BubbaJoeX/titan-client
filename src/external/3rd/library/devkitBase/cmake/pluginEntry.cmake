...
# Ensure DEVKIT_LOCATION is correctly formed with forward slashes
if(WIN32 AND DEFINED ENV{DEVKIT_LOCATION})
	cmake_path(SET FIXED_DEVKIT_LOCATION NORMALIZE $ENV{DEVKIT_LOCATION})
	set( ENV{DEVKIT_LOCATION} ${FIXED_DEVKIT_LOCATION} )
endif()
...
macro(find_tbb)
	if (INSIDE_MAYA)
		...
		# Check if TBB_ROOT_DIR is set as an environment variable and use it if set.
		# Otherwise, use the default TBB libs/headers from devkitBase.
      	if(NOT DEFINED ENV{TBBROOT})
			set(TBB_ROOT_DIR $ENV{DEVKIT_LOCATION})
		endif()
		find_package(TBB REQUIRED)
		if (TBB_FOUND)
			set(INCLUDE_DIRS ${INCLUDE_DIRS} ${TBB_INCLUDE_DIRS})
...
macro(find_qt libs)
	...
	# Check if the Qt archive has been expanded
	# We do not automatically expand the archive because it is too large
	if(NOT EXISTS $ENV{DEVKIT_LOCATION}/Qt)
		message( FATAL_ERROR "The Qt archive located in $ENV{DEVKIT_LOCATION} needs to be expanded before proceeding") 
	endif()

	# Locate Required Qt modules
	list( APPEND CMAKE_PREFIX_PATH "$ENV{DEVKIT_LOCATION}/Qt/lib/cmake" )
	find_package(Qt6 REQUIRED COMPONENTS ${libs} )

	# Initialize Qt CMake framework
	qt_standard_project_setup()
	set(CMAKE_AUTORCC ON) # Ensure that rcc gets run on Qt Resource files

	foreach(lib ${libs})
		set(PACKAGE_LIBS ${PACKAGE_LIBS} Qt6::${lib})
	endforeach()
endmacro()
...
function(build_plugin)
	...
	if (INSIDE_MAYA)
		...
		if (TARGET ${PROJECT_NAME} )
			# Target already created, just add the new source files if any
			target_sources(${PROJECT_NAME} PUBLIC ${SOURCE_FILES} )
			set( TARGET_ALREADY_DEFINED ON )
		elseif (IS_APPLICATION)
			if(LINUX)
