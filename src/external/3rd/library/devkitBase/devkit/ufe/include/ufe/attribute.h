...
public:
    ...
    //! \return The attributes nice display name. Calls name() by default.
    ...
    */
    ...
    //! The default implementation stores undo/redo data in an AttributeInfo that allows
    //! reconstructing a stale Attribute. Executing the command will throw if reconstruction
    //! fails.
...
public:
    ...
    //! \return Attribute current value.
    virtual std::string get() const = 0;

    //! Returns true if this attribute value was never set and is still at the
    //! default value.
    ...
    //! Return a command for undo / redo that sets the attribute value.
    //! The returned command is not executed; it is up to the caller to call
    //! execute().
    //! The default implementation stores undo/redo data in an AttributeInfo that allows
    //! reconstructing a stale Attribute. Executing the command will throw if reconstruction
    //! fails or leads to an incorrectly typed attribute.
    virtual UndoableCommand::Ptr setCmd(const std::string& value);

    //! Return a command for undo / redo that resets the attribute value.
    //! The returned command is not executed; it is up to the caller to call
    //! execute().
    //! The default implementation stores undo/redo data in an AttributeInfo that allows
    //! reconstructing a stale Attribute. Executing the command will throw if reconstruction
...
//! \typedef TypedAttribute<unsigned int> AttributeUInt
//! \brief Specialized typed attribute for integer values.
