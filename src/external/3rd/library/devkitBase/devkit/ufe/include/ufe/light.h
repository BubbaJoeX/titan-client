...
*/
    ...
    public:
        ...
        *************************************************/
        ...
        class UFE_SDK_DECL CylinderInterface
        {
        public:
            virtual ~CylinderInterface();

            //! Create an undoable command to set the cylinder volume properties. The command is not executed.
            //! \param radius the new cylinder volume radius.
            //! \param length the flag specifying the length of the cylinder
            //! \return Undoable command to set the volume properties.
            virtual VolumePropsUndoableCommand::Ptr volumePropsCmd(float radius, float length) = 0;

            //! Set the sphere properties. Default implementation uses VolumePropsUndoableCommand.
            //! \param radius the new cylinder radius.
            //! \param length the flag specifying the length of the cylinder
            virtual void volumeProps(float radius, float length) {
                if (auto cmd = volumePropsCmd(radius, length)) {
                    cmd->execute();
                }
            }

            //! Retrieve the volume properties for the light
            //! \return the volume properties
            virtual VolumeProps volumeProps() const = 0;
        };

        //! Retrieve the cylinder interface for the light
        //! \return the cylinder interface
        std::shared_ptr<CylinderInterface> cylinderInterface();

        /*************************************************
            Disk interface.
            Valid for the following light types: [Disk]
        *************************************************/

        class UFE_SDK_DECL DiskInterface
        {
        public:
            virtual ~DiskInterface();

            //! Create an undoable command to set the disk volume properties. The command is not executed.
            //! \param radius the new disk radius.
            //! \return Undoable command to set the volume properties.
            virtual VolumePropsUndoableCommand::Ptr volumePropsCmd(float radius) = 0;

            //! Set the disk volume properties. Default implementation uses VolumePropsUndoableCommand.
            //! \param radius the new disk radius.
            virtual void volumeProps(float radius) {
                if (auto cmd = volumePropsCmd(radius)) {
                    cmd->execute();
                }
            }
        ...
        /*************************************************
            Dome interface.
            Valid for the following light types: [Dome]
        *************************************************/

        class UFE_SDK_DECL DomeInterface
        {
        public:
            virtual ~DomeInterface();

            //! Create an undoable command to set the dome volume properties. The command is not executed.
            //! \param radius the new dome radius.
            //! \return Undoable command to set the volume properties.
            virtual VolumePropsUndoableCommand::Ptr volumePropsCmd(float radius) = 0;

            //! Set the dome volume properties. Default implementation uses VolumePropsUndoableCommand.
            //! \param radius the new dome radius.
            virtual void volumeProps(float radius) {
                if (auto cmd = volumePropsCmd(radius)) {
                    cmd->execute();
