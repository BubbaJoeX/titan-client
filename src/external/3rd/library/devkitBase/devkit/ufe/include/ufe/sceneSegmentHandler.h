...
public:
    ...
    */
    ...
    /*!
        Assuming a scene that has its root scene segment with the runtime ID of this scene segment handler,
        return the constant path that is the root of the root scene segment.
        By definition, this path will have a single path segment, since it describes the root scene segment.
        It will most likely have a small number of path components, often a single one, to represent the root of the scene graph.
        \return Single segment path describing the root of the scene.
    */
