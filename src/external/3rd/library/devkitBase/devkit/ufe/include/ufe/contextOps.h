...
struct UFE_SDK_DECL ContextItem {
    ...
    /*!
        ...
        Indicates that an item will be an option box item. 
        This item usually appears to the right of the preceding item.
    */
    ...
    // --------------------------------------------------------------------- //
    /// \name Meta Data Access:
    /// @{
    // --------------------------------------------------------------------- //

    /*!
        Get the value of the meta data named key.
        \param[in] key The meta data key to query.
        \return the value if the key exists in meta data. If the key does not exist an empty Ufe::Value is returned.
    */
    Ufe::Value getMetaData(const std::string& key) const;

    /*!
        Set the meta data key's value to value.
        \param[in] key The meta data key to set.
