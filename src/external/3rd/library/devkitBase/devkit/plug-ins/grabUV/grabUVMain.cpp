...
private:		   	 
	...
	double          fSizeInView;
	double          fSizeInViewSquare;

	bool getDataFromOptionVar(MFloatArray& positions, MFloatArray& values);
	MObject         fCurveAttr;
	MObject         fNode;
	MDoubleArray    fFallofWeight;
}; 
...
bool grabUVContext::getDataFromOptionVar(MFloatArray& positions, MFloatArray& values)
{
	// get the values from the option vars: "grabUV_curveControl"
	MStringArray ret;
	MString cmd = MString("optionVar -q grabUV_curveControl");
	MStatus res = MGlobal::executeCommand(cmd, ret, true);
	bool valid = (res == MS::kSuccess);
	...
	if (valid)
	{
		for (int i = 0; i < ret.length(); i++)
		{
			...
			if (ret[i].split(',', splitArray) != MS::kSuccess)
			{
				valid = false;
				break;
			}
			...
			double x = splitArray[0].asDouble();
			double y = splitArray[1].asDouble();
			positions.append(x);
			values.append(y);
		}
	}
...
MStatus	grabUVContext::doPress ( MEvent & event, MHWRender::MUIDrawManager& drawMgr, const MHWRender::MFrameContext& context)
{
	...
	fSizeInView = portW < 1e-5 ? 0.0 : ( fBrushConfig.size() * (right - left) / portW );
	fSizeInViewSquare = fSizeInView * fSizeInView;
	...
	if( fDragMode == kNormal )
	{
		...
		if (bSelect)
		{
			...
			fNode = fDagPath.transform();
			fDagPath.extendToShape();

			MFnMesh mesh(fDagPath);
			MCurveAttribute* falloffCurve = nullptr;

			// collect curve data from the "grabUV_curveControl" optionVar
			MFloatArray positions;
			MFloatArray values;
			if (getDataFromOptionVar(positions, values))
			{
				if (fCurveAttr.isNull()) 
				{
					MString nameLong("falloffCurve");
					MString nameShort("fc");
					MStatus status;
					fCurveAttr = MCurveAttribute::createCurve(nameLong, nameShort, fNode, values, positions, &status);
				}

				falloffCurve = new MCurveAttribute(fNode, fCurveAttr);
				falloffCurve->setCurve(values, positions);
			}
			...
			if( component.apiType() == MFn::kMeshMapComponent )
			{
				...
				for (unsigned int i = 0; i < UVsToTest.length(); ++i)
				{
					...
					if (bGetUV == MS::kSuccess)
					{
						...
						if (distSquare <= fSizeInViewSquare)
						{
				...
				// pre-compute falloff weights
				fFallofWeight.clear();
				for (unsigned int i = 0; i < fCollectedUVs.length(); i++)
					fFallofWeight.append(0.0);
				...
				if (falloffCurve)
				{
					for (unsigned int i = 0; i < fCollectedUVs.length(); i++)
					{
						...
						if (bGetUV == MS::kSuccess)
						{
							...
							{
								float value;
								double d = distSquare / fSizeInViewSquare;
								falloffCurve->getValueAtPosition((float)d, value);
								fFallofWeight[i] = value;
							}
						}
					}
				}
	...
	else
	{
		...
		if (fCommand)
		{
			...
			for (unsigned int i = 0; i < fCollectedUVs.length(); ++i)
			{
				...
				if (bGetUV == MS::kSuccess)
				{
					double falloffWeight = fFallofWeight[i];

					pinWeight = fullPinData[fCollectedUVs[i]];
					u += (float)vec[0]*(1-pinWeight)*(falloffWeight);
					v += (float)vec[1]*(1-pinWeight)*(falloffWeight);
					uValues.append( u );
					vValues.append( v ); 
				}
			}
