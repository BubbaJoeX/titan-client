...
*/
	...
	*/
    ...
    void mayaAfterSceneOpen(void* clientData )
    {
        dx11ShaderNode* node = (dx11ShaderNode*)clientData;
        if (node && node->getReloadShaderAfterSceneLoad() ){
            //Global MEL proc execution is not allowed when loading a scene for security reasons.
            // So reload the shader once we finished loading the file
            node->setCalledFromMayaAfterSceneOpenCallback(true);
            node->reload();
            node->setCalledFromMayaAfterSceneOpenCallback(false);
        }
    }
}
	...
	, fImplicitAmbientLight(-1)
{
	...
	fMayaAfterOpenSceneCallbackId = MSceneMessage::addCallback( MSceneMessage::kAfterOpen, mayaAfterSceneOpen, this);
...
dx11ShaderNode::~dx11ShaderNode()
{
	...
	resetData();
	fErrorLog.clear();
	MSceneMessage::removeCallback( fMayaAfterOpenSceneCallbackId );
}
		...
		case eScriptedTest:
			fTechniqueIsTransparent = eScriptedTest;
			{
					...
					procBody.length() > 0)
				{
					...
					printf("-- transparencyTest <<%s>>.\n", procBody.asChar());
#endif
					//Are we reading a scene file ?
					const bool readingSceneFile = MFileIO::isOpeningFile();
					//To execute the global MEL proc we need not to be reading a scene file or to be called from the maya after scene load callback
					//As when being in the MSceneMessage::kAfterOpen callback, readingSceneFile is still true but we want to execute the command in that case as it usually works
					const bool executeTheCommand = ( (! readingSceneFile) || fCalledFromMayaAfterSceneOpenCallback);
					if (executeTheCommand)
					{
						fReloadShaderAfterSceneLoad = false;

						// eval the body only once to define the procedure
						if (MS::kSuccess == MGlobal::executeCommand(procBody, false, false))
						{
							break;
						}
					}
					...
					{
						//When loading a scene, it happens that we cannot execute a global MEL proc for security reasons
						if (readingSceneFile)
						{
							//We are loading a scene, we cannot execute the global MEL proc for security reasons
							// Set fReloadShaderAfterSceneLoad to true so we reload the shader once the scene is loaded 
							// to work around this limitation
							fReloadShaderAfterSceneLoad = true;
						}
					}
				}
...
void dx11ShaderNode::refreshLightConnectionAttributes(bool inSceneUpdateNotification)
{
	...
	{
		// Hmmm. Really not a good idea to start adding parameters while scene is not fully loaded.
...
void dx11ShaderNode::updateImplicitLightParameterCache(std::vector<CUniformParameterBuilder*>& builders)
{
	...
	const bool readingOrOpeningFile = MFileIO::isReadingFile() || MFileIO::isOpeningFile();
	bool updateConnectionAttributes = ( ! readingOrOpeningFile);
	...
	//When reading a file, if fLightParameters has the same size as fLightNames, we don't clear the array as it has been initialized with the suitable lights bindings
	if ( ! readingOrOpeningFile || (fLightParameters.size() != fLightNames.length()) ) {
