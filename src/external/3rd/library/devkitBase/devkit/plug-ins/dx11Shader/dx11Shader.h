...
public:
	...
	//When loading a scene we cannot execute a global MEL proc for security reasons 
	// so we may have to reload the shader after load
	bool getReloadShaderAfterSceneLoad() const {return fReloadShaderAfterSceneLoad;}
	//Are we called from the maya after scene open callback ?
	void setCalledFromMayaAfterSceneOpenCallback(bool value) { fCalledFromMayaAfterSceneOpenCallback = value; }
	/////////////////////////////////
...
private:
	...
	//When loading a scene we cannot execute a global MEL proc for security reasons 
	// if that is the case fReloadShaderAfterSceneLoad is set to true to use the after scene load callback
	bool							fReloadShaderAfterSceneLoad {false};
	//Are we called from the maya after scene open callback ?
	bool							fCalledFromMayaAfterSceneOpenCallback {false};
	MCallbackId						fMayaAfterOpenSceneCallbackId {0};
