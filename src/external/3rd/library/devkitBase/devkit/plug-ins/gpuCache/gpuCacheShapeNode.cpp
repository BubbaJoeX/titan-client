...
bool ShapeNode::canMakeLive() const { 
    // About to be made Live, so cache standard accelerator now while on the main thread.  
    // At least this appears to be the best way to create our caches while still on the main thread.
    // This is basically an "About to be made Live" call, and there is no "pre-snapping to you"
    // call, so if your snapping implementation needs to be initialized on the main thread, then
    // this looks like the place to do it.
    //
    // Looks like all calls to getIntersectionAccelerator use these parameters, and they cache
    // the result, so calling this here in this way should be the thing to do.
    const double seconds = getEffectiveTime().as(MTime::kSeconds);
	gpuCacheIsectAccelParams accelParams = gpuCacheIsectAccelParams::autoUniformGridParams(); 
	getIntersectionAccelerator(accelParams, seconds);
