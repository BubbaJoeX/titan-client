	...
	public:
		...
		void initialize(const MHWRender::MRenderItem& renderItem) override
		{
			...
			if (fComponentType == MFn::kMeshPolygonComponent) {
				ApiMeshHWSelectionUserDataPtr selectionData = ApiMeshHWSelectionUserDataPtr::dynamic_pointer_cast(renderItem.getCustomData());
				if(selectionData) {
	...
	bool updateGeometry)
{
	...
	if (!fShadedShader)
	{
		fShadedShader = shaderMgr->getStockShader(MShaderManager::k3dIsotropicOpenPBRSurfaceShader);
	}
	...
	for (auto& p : fViewSelectedFaceInfoCache)
	{
		...
		for (int instIdx : instIdxArray)
		{
			...
			if (updateViewSelectedFaces || updateMaterial || updateGeometry)
			{
				...
				userData = ApiMeshHWSelectionUserDataPtr::dynamic_pointer_cast(viewSelectedShadedItem->getCustomData());				
				...
				userData = ApiMeshHWSelectionUserDataPtr::dynamic_pointer_cast(viewSelectedTexturedItem->getCustomData());
				if (userData)
				{
					userData->fMeshGeom = meshGeom;
				}

				userData = ApiMeshHWSelectionUserDataPtr::dynamic_pointer_cast(viewSelectedFaceSelectionItem->getCustomData());
				if (userData)
				{
					userData->fMeshGeom = meshGeom;
    ...
    MDagPath& dagPath) const
{
    ...
    if (instanceCount == 1 || instanceId == -1)
    {
        ...
        ApiMeshHWSelectionUserDataPtr userData = ApiMeshHWSelectionUserDataPtr::dynamic_pointer_cast(renderItem.getCustomData());
        if (userData &&
