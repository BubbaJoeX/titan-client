...
public:
    ...
    OPENMAYA_ENUM(MIdleTaskPriority,
		kHighIdlePriority,		//!< High priority \note High priority tasks are executed in last-in first-out order
		kLowIdlePriority,		//!< Low priority \note This is the default idle priority
		kVeryLowIdlePriority	//!< Very low priority
	);
	
	static MStatus 		executeCommandOnIdle( const MString& command,
										bool displayEnabled = false);
	OPENMAYA_AVAILABLE(2026.1)
	static MStatus 		executeCommandOnIdleWithPriority( const MString& command,
										MIdleTaskPriority priority,
										bool displayEnabled = false);
...
END_NO_SCRIPT_SUPPORT:
	...
	OPENMAYA_AVAILABLE(2026.1)
	static MStatus 		executePythonCommandOnIdleWithPriority( const MString& command,
												MIdleTaskPriority priority,
												bool displayEnabled = false);
